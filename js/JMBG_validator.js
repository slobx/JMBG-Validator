...
function checkJMBG() {
    var input_jmbg = document.getElementById("inputJMBG").value;

    if (input_jmbg.toString().length == 13) {
        if (validateJMBG(input_jmbg)) {

            calculateJMBG(input_jmbg);

        } else {
            alert("Number/JMBG is not valid");
        }

    } else {
        alert("Number/JMBG must have 13 digits");
    }
}
...
function calculateJMBG(JMBG) {
    ...
    document.getElementById("birth_date").innerHTML = output[0] + "" + output[1] + " " + returnMonth(output[2] + "" + output[3]) + " " + returnYear(output[4]) + "" + output[5] + "" + output[6];
    document.getElementById("birthplace").innerHTML = returnPlace(output[7] + "" + output[8]);
    document.getElementById("sex").innerHTML = returnSex(sexNum);
...
function returnPlace(num) {
    switch (num) {
        ...
        case "35":
            ...
            break;
        case "36":
            return "Rijeka, Pula, Istra and Primorje regions";
            break;
        case "37":
        ...
        case "50":
            ...
            //Centralna Srbija
        case "71":
            return "Beograd region (Grad Beograd: Stari Grad, Savski Venac, Voždovac, Vračar, Palilula, Zvezdara, Rakovica, Čukarica, Novi Beograd, Zemun, Mladenovac, Barajevo, Grocka, Obrenovac, Sopot, Lazarevac)";
            break;
        case "72":
            return "Šumadija i Pomoravlje regions (Šumadijski okrug: Aranđelovac, Batočina, Knić, Kragujevac, Rača, Lapovo, Topola), (Pomoravski okrug: Despotovac, Paraćin, Rekovac, Jagodina, Svilajnac, Ćuprija)";
            break;
        case "73":
        ...
        case "76":
            ...
            break;
        case "77":
            return "Podrinje i Kolubara regions (Mačvanski okrug: Loznica, Krupanj, Ljubovija, Šabac, Bogatić, Koceljeva, Vladimirci, Mali Zvornik), (Kolubarski okrug: Valjevo, Lajkovac, Ljig, Ub, Osečina, Mionoca)";
            break;
        case "78":
